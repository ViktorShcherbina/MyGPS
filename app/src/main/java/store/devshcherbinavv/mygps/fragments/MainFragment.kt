...
import store.devshcherbinavv.mygps.databinding.FragmentMainBinding
import store.devshcherbinavv.mygps.db.TrackItem
import store.devshcherbinavv.mygps.location.LocationModel
import store.devshcherbinavv.mygps.location.LocationService
import store.devshcherbinavv.mygps.utils.DialogManager
import store.devshcherbinavv.mygps.utils.TimeUtils
import store.devshcherbinavv.mygps.utils.checkPermission
...
class MainFragment : Fragment() {
    ...
    private fun setOnClicks() = with(binding){
        ...
        ibZoomIn.setOnClickListener(listener)
        ibZoomOut.setOnClickListener(listener)
    }
    ...
    private fun onClicks(): View.OnClickListener{
        return View.OnClickListener{
            when(it.id) {
                ...
                R.id.ibZoomIn -> zoomInLocation()
                R.id.ibZoomOut -> zoomOutLocation()
            }
        }

    }

    private fun zoomInLocation(){
        binding.map.controller.zoomIn()
    }

    private fun zoomOutLocation(){
    ...
    private fun locationUpdates() = with(binding){
        model.locationUpdates.observe(viewLifecycleOwner){
           ...
           val aVelocity = "Average velocity: ${getAverageVelocity(it.distance)} km/h"
    ...
    private fun registerPermissions(){
        pLauncher = registerForActivityResult(
            ...
            if (it[Manifest.permission.ACCESS_FINE_LOCATION] == true){
                ...
                showToast("You have not given permission to use the location!")
            }
        }
    }
